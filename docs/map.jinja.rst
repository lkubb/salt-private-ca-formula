...
We will see a quick setup to configure the ``pca`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``pca`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``pca`` formulas under ``/srv/salt/pca/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/pca/parameters/dns:domain/
    ln -s dns:domain /srv/salt/pca/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/pca/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/pca/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``pca`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``pca`` formulas under ``/srv/salt/pca/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/pca/parameters/roles
...
We create a configuration for the role ``pca/server`` in ``/srv/salt/pca/parameters/roles/pca/server.yaml``:
...
We create another configuration for the role ``pca/client`` in ``/srv/salt/pca/parameters/roles/pca/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles pca/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles pca/client
...
.. code-block:: console
    ...
    mv /srv/salt/pca/parameters/dns:domain/  '/srv/salt/pca/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-pca-as-json:
      test.nop:
        - name: {{ pca | json }}
...
Here is an example based on `template-formula/pca/config/file.sls`_:
    ...
    pca-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='pca-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``pca`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ pca['winner'] }}
...
.. _template-formula/pca/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/pca/config/file.sls
